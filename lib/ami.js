    ...
    if (!lines.length && (line.substr(0, 21) === 'Asterisk Call Manager')) {
      ...
      while (lines.length) {
        ...
        if (key === 'variable' || key === 'chanvariable') {
          ...
          if (typeof(item[key]) !== 'object') { item[key] = item[key] || {}; }
          line = line.split('=');
...
function ManagerKeepConnected(context) {
  ...
  var connect = this.connect.bind(
    this, this.options.port,
    this.options.host,
    this.login.bind(this)
  );
  var backoff = 0;
  this.on('connect', clearReconnection);
  ...
  function clearReconnection() { backoff = 0; }
}
...
function ManagerDisconnect(context, callback) {
  ...
  if (this.reconnect)
    this.removeListener('on', this.reconnect);
