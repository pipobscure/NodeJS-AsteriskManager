...
var Manager = function Manager(port, host, username, password, events) {
  ...
  obj.options = {port: port, host: host, username: username, password: password, events: events || false};
...
function ManagerAction(context, action, callback) {
  ...
  try {
    if (!context.connection) throw new Error('There is no connection yet');
